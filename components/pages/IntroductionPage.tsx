...
export function IntroductionPage() {
  ...
  const [demoContent, setDemoContent] = useState<any>({ title: "", description: "", items: [] }); // Demo 상태 추가
    ...
    const fetchPageData = async () => {
      try {
        ...
        if (data?.content) {
          ...
          if (data.content.demo) {
            setDemoContent(data.content.demo);
          }
        }
      } catch (e) {
  ...
  return (
    <SmoothScroll>
      <main className="bg-black min-h-screen text-white selection:bg-cyan-500 selection:text-black overflow-x-hidden">
        ...
        {/* Section 4: Demo */}
        <Section4_Demo
          title={demoContent.title || STATIC_CONTENT.demo.title[language]}
          description={demoContent.description || STATIC_CONTENT.demo.description[language]}
